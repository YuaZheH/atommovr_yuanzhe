...
class AtomArray:
    ...
    def move_atoms(self, move_list: list) -> list:
        ...
        for move in moves_wo_crossing:
            dist = move.distance*self.params.spacing + (self.error_model.putdown_time + self.error_model.pickup_time)*self.params.AOD_speed
    ...
    def _apply_moves(self, 
                     moves: list, 
                     duplicate_move_inds: list = []) -> tuple[list, list]:
        """ 
    ...
    def _apply_moves_single_species(self,
                                    moves: list, 
                                    duplicate_move_inds: list = []) -> tuple[list, list]:
        """ 
        Applies moves to an array of atoms (represented by `matrix_out`). 
        ...
        for move_ind, move in enumerate(moves):
            ...
            if move_ind in duplicate_move_inds:
                ...
                elif move.movetype == MoveType.LEGAL_MOVE or move.movetype == MoveType.ILLEGAL_MOVE:
                    if state_before_moves[move.from_row][move.from_col] > 0:
                        self.matrix[move.from_row][move.from_col] -= 1
    ...
    def _apply_moves_dual_species(self, 
                                  moves: list, 
                                  duplicate_move_inds: list = []) -> tuple[list, list]:
        """ 
            ...
            if move_ind in duplicate_move_inds:
                ...
                if int(np.sum(self.matrix[move.from_row,move.from_col,:])) == 1:
                    try:
                        ...
                        if int(np.sum(self.matrix[move.to_row,move.to_col,:])) == 0 and move.to_col >= 0 and move.to_row >= 0:
                            move.movetype = MoveType.LEGAL_MOVE
                        # check if there is an atom in the putdown pos
                        elif int(np.sum(self.matrix[move.to_row,move.to_col,:])) == 1 and move.to_col >= 0 and move.to_row >= 0:
                            move.movetype = MoveType.ILLEGAL_MOVE
                        elif move.to_col >= 0 and move.to_row >= 0:
                            raise Exception(f"{int(np.sum(self.matrix[move.to_row,move.to_col,:]))} is not a valid matrix entry.")
                        else:
                            raise IndexError
                    except IndexError:
                        move.movetype = MoveType.EJECT_MOVE
                ...
                if move.failure_flag != 0:
                    ...
                    flags.append(move.failure_flag)
                    if move.failure_flag == 2: #PUTDOWNFAIL, see above
                        if int(np.sum(self.matrix[move.from_row,move.from_col,:])) == 0:
                            raise Exception(f"Error occured in MoveType. There is NO atom at ({move.from_row}, {move.to_row}).")                
                        self.matrix[move.from_row][move.from_col][0] = 0
                        self.matrix[move.from_row][move.from_col][1] = 0
                ...
                # elif the move is an ejection move, remove the atom(s)
                    ...
                    if np.sum(self.matrix[move.from_row,move.from_col,:]) == 0:
                        raise Exception(f"Error occured in MoveType assignment. There is NO atom at ({move.from_row}, {move.to_row}).")
                    # Eject Rb atom
                    if self.matrix[move.from_row][move.from_col][0] > 0:
                        self.matrix[move.from_row][move.from_col][0] -= 1
                    # Eject Cs atom
