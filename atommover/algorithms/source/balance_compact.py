...
def balance_and_compact(init_config: np.ndarray, target_config: np.ndarray, do_ejection : bool = False, final_size: list = []):
    if len(np.shape(init_config)) > 2 and np.shape(init_config)[2] == 1:
        ...
        raise ValueError(f'Atom array has shape {np.shape(init_config)}, which is not correct for single species. Did you meant to use a dual species algorithm?')
    success_flag = False
    ...
    return final_config, move_list, success_flag #, [balance_moves_term, compact_moves_term, ejection_moves_term]
...
def balance_bc(matrix, target_config, row_min, row_max, col_min, col_max, move_list, recursive_flag):
    ...
    if recursive_flag == 0:
        balance_config, move_list = balance_bc(balance_config, target_config, row_min, middle_row, col_min, col_max, move_list, 0)
        balance_config, move_list = balance_bc(balance_config, target_config, middle_row + 1, row_max, col_min, col_max, move_list, 0)
...
def down_move(matrix, target_config, excess_atoms, row_min, middle_row, row_max, col_min, col_max, move_list):
    ...
    while row_max >= source_row >= row_min and excess_atoms > 0:
        ...
        if sum(matrix[target_row,col_min:col_max+1]) == len(matrix[target_row,col_min:col_max+1]) and target_row + stuff + 1 < len(matrix) and excess_atoms > 0:
    ...
    while success_flag == False:
        ...
        for col_ind in range(middle_col - 1,-1,-1):
            ...
            for row_ind, spot in enumerate(col):
                num_target_spots = np.sum(target_config[row_ind, col_ind:middle_col+1])
                num_atoms = np.sum(matrix[row_ind, col_ind:middle_col+1])
                if spot == 0 or num_target_spots < num_atoms or matrix[row_ind, col_ind + 1] == 1:
                    pass
                else:
...
def compact_right(init_matrix, target_config, middle_col, move_list, max_cycles = 30):
    ...
    while success_flag == False:
        ...
        for col_ind in range(middle_col + 2, len(matrix[0])):
            ...
            for row_ind, spot in enumerate(col):
                num_target_spots = np.sum(target_config[row_ind,middle_col + 1:col_ind])
