...
def calculate_LB(atom_arrays: np.ndarray, target_config: np.ndarray, n_species: int=1):
...
def get_LBAP_pairing(atom_arrays: np.ndarray, target_config: np.ndarray, n_species=1, metric = 'euclidean'):
    mat_copy = copy.deepcopy(atom_arrays)
    ...
    for species_ind in range(np.shape(mat_copy)[2]):
        ...
        # Generate the cost matrix using the current atom positions and the target positions
        cost_matrix = generate_cost_matrix(current_positions, target_positions, metric = metric)
...
def calculate_Zstar_better(atom_arrays: np.ndarray, target_config: np.ndarray, n_species=1, metric = 'euclidean'):
    mat_copy = copy.deepcopy(atom_arrays)
    targ_copy = copy.deepcopy(target_config)
    if n_species == 1:
        mat_copy.reshape([len(mat_copy), len(mat_copy[0]), 1])
    ...
    for species_ind in range(np.shape(mat_copy)[2]):
        ...
        #Define target positions for the center square in a matrix.
        current_positions, target_positions = define_current_and_target_new(matrix, target)
    ...
    for species_ind in range(np.shape(mat_copy)[2]):
        ...
        if not zstar_fail_flag:
            ...
            if LB >largest_distance:
                raise Exception(f'Algorithm error. Lower bound {LB} cannot be larger than Z* {largest_distance}.')
        else:
            largest_distance = 0
...
def define_current_and_target(matrix, target_config):
    current_positions = [(x, y) for x in range(len(matrix[0])) for y in range(len(matrix)) if matrix[y][x] == 1 if target_config[y][x] == 0] # NKH: this used to be matrix[x,y] and target_config[x,y (the row index and column index were mixed up) but I fixed it
    target_positions = [(x, y) for x in range(len(matrix[0])) for y in range(len(matrix)) if matrix[y][x] == 0 if target_config[y][x] == 1] # NKH: same here
...
def define_current_and_target_new(matrix, target_config):
    ...
    current_positions = [(x, y) for x in range(len(matrix[0])) for y in range(len(matrix)) if matrix[y][x] == 1]# if target_config[y][x] == 0] # NKH: this used to be matrix[x,y] and target_config[x,y (the row index and column index were mixed up) but I fixed it
    target_positions = [(x, y) for x in range(len(matrix[0])) for y in range(len(matrix)) if target_config[y][x] == 1]# if matrix[y][x] == 0] # NKH: same here
    return current_positions, target_positions
...
def transform_cost_matrix_into_integers(matrix):
    unique_vals, unique_inverse, unique_counts = np.unique(matrix, return_inverse=True, return_counts=True)
    new_vals = np.array(range(1,len(unique_vals)+1))
    return new_vals[unique_inverse].reshape(np.shape(matrix)), unique_inverse, unique_counts
...
def convert_LBAP_to_LSAP(matrix):
    ...
    ranked_mat, unique_inverse, Snorm_array = transform_cost_matrix_into_integers(simp_mat)
