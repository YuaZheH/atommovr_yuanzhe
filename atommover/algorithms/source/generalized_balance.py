...
def generalized_balance(init_config, target_config, do_ejection: bool = False, final_size: list = []):
    ...
    balance_config, move_list = row_balance(matrix, target_config, row_min, row_max, col_min, col_max, move_list, 0)
...
def row_balance(matrix, target_config, row_min, row_max, col_min, col_max, move_list, recursive_flag):
    ...
    if N_S_i_m > n_req and N_S_i_m_c < n_req_c:
        ...
        move_list, balance_config = down_move(matrix, target_config, excess_atoms, row_min, middle_row, row_max, col_min, col_max, move_list)
        ...
        move_list, balance_config = up_move(matrix, target_config, needed_atoms, row_min, middle_row, row_max, col_min, col_max, move_list)
        #return matrix, move_list
    else:
        balance_config = matrix

    if recursive_flag == 0:
        balance_config, move_list = col_balance(balance_config, target_config, row_min, middle_row, col_min, col_max, move_list, 0)
        balance_config, move_list = col_balance(balance_config, target_config, middle_row + 1, row_max, col_min, col_max, move_list, 0)

    return balance_config, move_list

def col_balance(matrix, target_config, row_min, row_max, col_min, col_max, move_list, recursive_flag):
    # Recursively balance the submatrices
    col_nums = col_max - col_min + 1

    if col_nums == 1 and recursive_flag == 0:
        return row_balance(matrix, target_config, row_min, row_max, col_min, col_max, move_list, 1)

    middle_col = col_min + (col_nums // 2) - 1
    n_req = int(np.sum(target_config[row_min:row_max + 1, col_min:middle_col + 1]))
    n_req_c = int(np.sum(target_config[row_min:row_max + 1, middle_col + 1:col_max + 1]))
    #print(f"n_req: {n_req}")
    ...
    if N_S_i_m > n_req and N_S_i_m_c < n_req_c:
        # Shift excess 1s down from S[i:m+1] to S[m+1:j+1]
        excess_atoms = min(N_S_i_m - n_req, n_req_c - N_S_i_m_c)
        move_list, balance_config = right_move(matrix, target_config, excess_atoms, row_min, row_max, col_min, middle_col, col_max, move_list)
        #return matrix, move_list

    elif N_S_i_m < n_req and N_S_i_m_c > n_req_c:
        # Shift required 1s up from S[m+1:j+1] to S[i:m+1]
        needed_atoms = min(n_req - N_S_i_m, N_S_i_m_c - n_req_c)
        move_list, balance_config = left_move(matrix, target_config, needed_atoms, row_min, row_max, col_min, middle_col, col_max, move_list)
        #return matrix, move_list
    else:
        balance_config = matrix

    if recursive_flag ==0:
        balance_config, move_list = row_balance(balance_config, target_config, row_min, row_max, col_min, middle_col, move_list, 0)
        balance_config, move_list = row_balance(balance_config, target_config, row_min, row_max, middle_col + 1, col_max, move_list, 0)
...
def right_move(matrix, target_config, excess_atoms, row_min, row_max, col_min, middle_col, col_max, move_list):
    #Initialize the move bound
    ...
    while col_max >= source_col >= col_min and excess_atoms > 0:
        ...
        for row in range(row_min,row_max + 1):
            if matrix[row, source_col + normalize_col] == 1 and matrix[row, source_col + normalize_col + 1] == 0 and matrix[row, target_col] == 0 and excess_atoms > 0:
                move = Move(row, source_col + normalize_col, row, source_col + normalize_col + 1)
                moves_in_scan.append(move)
        ...
        #TODO: Solve the stuff case later
        if sum(matrix[row_min:row_max+1,target_col]) == len(matrix[row_min:row_max+1,target_col]) and target_col + stuff < len(matrix) and excess_atoms > 0:
            for shift in range(stuff, -1, -1):
                moves_in_scan = []
                for row in range(row_min,row_max+1):
                    if matrix[row, target_col + shift] == 1 and matrix[row, target_col + shift + 1] == 0:
                        move = Move(row, target_col + shift, row, target_col + shift + 1)
                        moves_in_scan.append(move)
...
def left_move(matrix, target_config, excess_atoms, row_min, row_max, col_min, middle_col, col_max, move_list):
    # Initialize the move bound
    ...
    while col_max >= source_col >= col_min and excess_atoms > 0:
        ...
        if sum(matrix[row_min:row_max+1,target_col]) == len(matrix[row_min:row_max+1,target_col]) and target_col > 0 and excess_atoms > 0:
            for shift in range(stuff, -1, -1):
                moves_in_scan = []
                for row in range(row_min,row_max+1):
                    if matrix[row, target_col - shift] == 1 and matrix[row, target_col - shift - 1] == 0:
